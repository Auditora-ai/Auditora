...
const DEFAULT_COLOR = { bg: "bg-slate-50", border: "border-slate-300", text: "text-slate-800" };
	...
	if (nodes.length === 0) {
		return (
			<ReportSection title="Mapa del Proceso">
				<p className="text-sm text-slate-400">No se extrajeron actividades en esta sesion.</p>
			</ReportSection>
		);
	}
		...
		<ReportSection title="Mapa del Proceso">
			...
			<div className="space-y-3 overflow-x-auto">
				{Array.from(laneMap.entries()).map(([lane, laneNodes]) => (
					<div key={lane} className="rounded-lg border border-slate-200 overflow-hidden">
						{/* Lane header */}
						<div className="bg-slate-100 px-4 py-2">
							<span className="text-sm font-medium text-slate-700">{lane}</span>
