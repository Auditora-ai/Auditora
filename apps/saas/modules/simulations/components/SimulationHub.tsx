...
export function SimulationHub({ templates, recentRuns, organizationSlug }: SimulationHubProps) {
	if (templates.length === 0) {
		return (
			<EmptyState
				icon={GraduationCapIcon}
				title="Sin simulaciones creadas"
				description="Las simulaciones se generan a partir de tus procesos y riesgos documentados. Primero mapea un proceso y detecta sus riesgos."
				actions={[
					{
						label: "Ir a Procesos",
		...
		<div className="flex flex-col gap-6">
			...
			<div className="grid grid-cols-3 gap-4">
				<div className="rounded-lg border border-border/50 bg-card p-4">
					...
					<p className="mt-1 text-2xl font-semibold text-foreground">{templates.length}</p>
				</div>
				<div className="rounded-lg border border-border/50 bg-card p-4">
				...
				<div className="grid gap-3">
					{templates.map((template) => {
						...
						return (
							<Link
								key={template.id}
								href={`/${organizationSlug}/simulations/${template.id}`}
								className="group flex items-center justify-between rounded-lg border border-border/50 bg-card p-4 transition-colors hover:border-border hover:bg-accent/30"
							>
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
											...
											<span>{roleName}</span>
											<span>·</span>
											<span>{runCount} evaluaciones</span>
										</div>
									</div>
								</div>
						...
						{recentRuns.map((run) => (
							...
							>
								<div className="flex items-center gap-3">
									...
									<div>
										<span className="text-sm text-foreground">{run.user.name}</span>
								...
								<div className="flex items-center gap-3">
									<span className={cn("text-sm font-medium", scoreColor(run.overallScore))}>
									...
									<span className={cn(
										...
										{run.status === "COMPLETED" ? "Completada" : run.status === "IN_PROGRESS" ? "En curso" : "Abandonada"}
									</span>
								</div>
							</div>
						))}
