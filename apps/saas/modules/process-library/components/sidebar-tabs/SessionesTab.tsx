		...
		return (
			<div className="flex flex-col items-center justify-center py-8 text-center">
				...
				<Button size="sm" asChild>
					<Link href={`/${organizationSlug}/capture/new?processId=${processId}&type=DEEP_DIVE`}>
			...
			<div className="flex gap-1.5 mb-3">
				{lastSession?.status === "ENDED" && (
					<Button variant="secondary" size="sm" className="text-xs h-7 flex-1" asChild>
						<Link href={`/${organizationSlug}/capture/new?processId=${processId}&type=DEEP_DIVE&continuationOf=${lastSession.id}`}>
							Continuar
						</Link>
					</Button>
				)}
